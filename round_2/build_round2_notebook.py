...
def build_round2_notebook():
    ...
    cells.append(cell_code([
        ...
        "# Helper to locate datasets whether run from root or round_2/",
        "def find_data(filename):",
        "    for p in [filename, os.path.join('round_2', filename), os.path.join('round_1', filename), os.path.join('..', filename)]:",
        "        if os.path.exists(p): return p",
        "    return filename",
        "",
        "# Load datasets",
        "posts_cleaned = pd.read_csv(find_data('Social_Engine_Posts_Cleaned.csv'))",
        "users_cleaned = pd.read_csv(find_data('Social_Engine_Users_Cleaned.csv'))",
        "posts_corrupted = pd.read_csv(find_data('Social_Engine_Posts_Corrupted.csv'))",
        "",
